---
name: openspec-siraj-create-plan
description: >
  ...
  and define the canonical 8-section shape that openspec-siraj-execute-plan
...
and the per-commit execution (`openspec-siraj-execute-plan`). They describe
...
**Where this fits:** Stage 6 of the openspec-siraj pipeline — invoked after `openspec-siraj-stress-test-tasks` clears, before `openspec-siraj-execute-plan` consumes the plan. Produces ONE per-commit plan file at a time. See `openspec-siraj-flow` for the full pipeline diagram, sibling-skills graph, and the artifact-tree convention. After authoring, run `/review-document` for the structural lens pass; for load-bearing commits, `openspec-siraj-stress-test-plan` empirically validates the plan before execution.
...
- Authoring `spec.md` or `design.md` — separate concerns
  (`openspec-siraj-create-design` for design; spec is hand-edited per
...
- Does not implement anything — that's `openspec-siraj-execute-plan`.
...
- Does not implement the section — that's `openspec-siraj-execute-plan`, which runs as its own pipeline step against a *committed* plan.
...
This section is filled by `openspec-siraj-execute-plan`, not by the plan
...
The plan file feeds `openspec-siraj-execute-plan`. Each section serves a different read by the executor:
...
`/openspec-siraj-stress-test-plan`, this skill commits the plan file
...
3. **Propose the commit message** to the user:
   ...
   `openspec-siraj-flow §Git commit conventions`).
...
5. **Hand off:**
   ...
   > "Plan committed. Next: `/openspec-siraj-execute-plan` will
...
ONE commit from `openspec-siraj-execute-plan` (impl files + tasks.md
...
**Next:** `/review-document` on the freshly-authored plan (structural lens pass), then for load-bearing sections `/openspec-siraj-stress-test-plan` (empirical validation), then the §Commit the plan step above, then `/openspec-siraj-execute-plan` for the implementation. See `openspec-siraj-flow` for the full Stage 6 pipeline.
