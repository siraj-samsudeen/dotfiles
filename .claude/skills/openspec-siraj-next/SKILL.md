---
name: openspec-siraj-next
description: >
  Single entry point for the openspec-siraj pipeline. Reads disk state for the
  current OpenSpec change, identifies which artifact or step is needed next,
  and invokes the right verb — either upstream CLI (`/openspec-siraj:new`,
  `/openspec-siraj:continue`) or a siraj custom skill (`create-design`,
  ...
  pipeline state table. Trigger on `/openspec-siraj-next`, "what's next",
...
**Why this skill exists:** the openspec-siraj pipeline has ~8 verbs
...
- `/openspec-siraj-next` typed explicitly.
...
- The user explicitly asks for a SPECIFIC verb (e.g., "create the plan
  for §3" → go straight to `openspec-siraj-create-plan`, no need to
  detect state).
...
```
1. openspec/ exists?
   NO  → state: NO_CHANGE  → /openspec-siraj:new <name>
   YES → continue
...
3. Inside the change folder, walk artifact existence:
   proposal.md exists?              NO → state: NO_PROPOSAL  → /openspec-siraj:continue
   specs/<cap>/spec.md exists?       NO → state: NO_SPEC      → /openspec-siraj:continue
   design.md exists?                  NO → state: NO_DESIGN    → /openspec-siraj-create-design
   tasks.md exists?                   NO → state: NO_TASKS     → /openspec-siraj:continue

4. Per-section loop: parse tasks.md for the first ## N. <subject>
   whose checkboxes are not all [x]:
   plans/<N>-<slug>.md exists?       NO → state: NO_PLAN_§N    → /openspec-siraj-create-plan
   Plan file committed?              NO → state: PLAN_UNCOMMITTED  → run create-plan §Commit step
   Impl committed (any §N tick)?     NO → state: NO_IMPL_§N    → /openspec-siraj-execute-plan
   All §N boxes ticked AND committed? continue to next section

5. All sections complete (all tasks.md boxes ticked + committed)?
   YES → state: READY_VERIFY        → /openspec-siraj:verify, then :archive, then :sync
```
...
|---|---|---|
| No `openspec/` dir | `/openspec-siraj:new <name>` | Upstream CLI scaffolds the folder + proposal skeleton |
| `proposal.md` only | `/openspec-siraj:continue` | Upstream produces spec.md in its default shape; no siraj override needed |
| Proposal + spec, no `design.md` | `/openspec-siraj-create-design` | **Siraj override** — Format A walk-through replaces upstream's generic decisions section |
| Proposal + spec + design, no `tasks.md` | `/openspec-siraj:continue` | Upstream produces tasks.md in minimal shape — accepted as-is per the [[skill-authoring]] principle that tasks.md is the thin index |
| Tasks exist, no `plans/<N>-*.md` for next unticked § | `/openspec-siraj-create-plan` | **Siraj override** — 8-section canonical plan shape |
| Plan exists for §N, not yet committed | create-plan's §Commit step | Plan commit is its own pipeline step, distinct from impl |
| Plan committed, no impl for §N | `/openspec-siraj-execute-plan` | **Siraj override** — per-section RED-GREEN-coverage flow with explicit user-approved impl commit |
| All `tasks.md` boxes ticked + committed | `/openspec-siraj:verify` → `:archive` → `:sync` | Upstream CLI verbs are the canonical finish |
...
discipline takes over entirely. See the orchestrator `openspec-siraj-flow`
...
Run if:
- Fresh session start.
- Routing to `openspec-siraj-execute-plan` (its own Session Start does
...
| Re-implement what `openspec-siraj-flow`'s state table already encodes | This skill IS the state table made executable; keep the same routing in sync |
...
stage. See `openspec-siraj-flow` for the canonical pipeline diagram and
