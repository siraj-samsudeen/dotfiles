...
- Have an implementation plan (from `write-plan-from-spec`)
- Tasks are mostly independent
- Want to stay in this session (vs parallel session with `execute-plan`)
...
4. Use verify-feature skill
5. Use finish-branch skill
```
...
2. Use `verify-feature` skill (Layer 1-3 verification)
3. Use `finish-branch` skill (merge/PR/keep/discard)

## Relationship to Other Skills

```
write-plan-from-spec  →  execute-with-agents  →  verify-feature  →  finish-branch
                               ↓
                    (per task: implement → spec review → code review)
```

**Alternative:** Use `execute-plan` for parallel session with human checkpoints between batches.
