...
- TDD Guard hook active (from `/setup-tdd-guard`)
...
Use `/create-spec` for this slice only:
...
Use `/verify-feature` with 100% coverage gate:
...
/feather:work-slice (THIS SKILL)
     │
     ├── Invokes: /create-spec
     ├── Invokes: /derive-tests-from-spec
     ├── Invokes: /verify-feature
     │
     ├── Requires: /setup-tdd-guard active
     │
     ↓
