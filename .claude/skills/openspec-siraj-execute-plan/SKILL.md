---
name: openspec-siraj-execute-plan
description: >
  ...
  session start and routes the user to /openspec-siraj-create-plan if
...
upstream verb `openspec-siraj-create-plan` authors and commits the plan
...
Stage 6 of the openspec-siraj pipeline — runs AFTER `openspec-siraj-create-plan` has authored and committed the per-section plan file (and after `review-document` / `openspec-siraj-stress-test-plan` have polished it). Consumes the plan file at `openspec/changes/<change>/plans/<N>-<slug>.md`. See `openspec-siraj-flow` for the full pipeline diagram and the disambiguation between this skill and `/openspec-siraj:apply` (which this skill replaces) or `/openspec-siraj:verify` (post-implementation CLI gate).

## When to use

- A change folder has a polished plan file for the next unticked section, and you're ready to land the implementation.
- The user says "execute the next section", "implement section N", "/openspec-siraj-execute-plan", or pauses on a plan file with no impl yet.

## When NOT to use

- **The plan file for the next unticked section does not exist** — this skill halts at pre-flight; use `openspec-siraj-create-plan` first.
- The plan exists but hasn't been polished — run `review-document` and (for load-bearing sections) `openspec-siraj-stress-test-plan` first.
- Run-to-blocker bulk implementation without per-section review — use `/openspec-siraj:apply` (project-local policy avoids this; see orchestrator's "Avoided CLI verbs").
- Implementation done, checking against artifacts — use `/openspec-siraj:verify`.

## What this skill does NOT do

- **Does not author plan files.** That's `openspec-siraj-create-plan`. If the plan is missing, this skill halts and routes the user to create-plan.
- Does not validate cross-doc consistency — `openspec-siraj-review-change` does, pre-implementation.
- Does not check implementation against artifacts after the fact — `/openspec-siraj:verify` does.
...
3. **Pre-flight: does `plans/<N>-<slug>.md` exist?** If NO, **halt**
   ...
   ──────────────────────────────────────────────────────
     ...
     /openspec-siraj-create-plan first to author and commit
...
For each unticked ## N. <subject> in tasks.md:
  ...
  │ /openspec-siraj-create-plan first to author       │
  │ its plan, then invokes this skill again.        │
  └──────────────────────────────────────────────────┘
```

Per section: **one commit** from this skill (the impl). The plan
commit lands upstream from `openspec-siraj-create-plan`. Two commits
...
7. **Cleanup.** `git stash drop`, `git worktree remove -f -f` both
...
9. Cleanup (drop stash, remove worktree, prune branches).
...
> "§<N> committed.
> (a) stop here (default) — to do §<N+1>, run /openspec-siraj-create-plan
...
> "All sections complete. Want me to run `/openspec-siraj:verify` to
> check implementation against the spec? Once that's clean, archive
> with `/openspec-siraj:archive`."

Do not auto-run either.

**Next:** `/openspec-siraj:verify` to validate, then
`/openspec-siraj:archive`, then `/openspec-siraj:sync` to move delta
specs into `openspec/specs/`. See `openspec-siraj-flow` for the full
Stage 7 pipeline.
...
| ❌ Don't | ✅ Do |
|---|---|
| Author a plan file inside execute-plan | Halt at pre-flight; route the user to `/openspec-siraj-create-plan` |
