---
name: siraj-openspec-flow-is-canonical
description: siraj-openspec-flow skill is the canonical home for all OpenSpec spec/tasks/numbering/title/cross-ref conventions; openspec/config.yaml and openspec/conventions.md were deleted as redundant
metadata: 
  node_type: memory
  type: feedback
...
**How to apply:**
- Before drafting any OpenSpec artifact (spec.md, tasks.md, plan files), the canonical shape lives in `siraj-openspec-flow` (or `siraj-openspec-create-plan` for per-commit plans). Sub-skills like `siraj-openspec-create-design` carry only their own procedure + a "Where this fits" pointer back to the orchestrator.
...
**Caveat about `openspec update`:** the openspec CLI will recreate `openspec/config.yaml` and the `opsx/` command folder on each `openspec update` run. Delete the regenerated config.yaml and rename `opsx/` → `siraj-openspec/` afterward. Tracked as a known regeneration chore.
